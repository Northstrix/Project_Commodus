...
void send_data(int line, int disp){
   ...
   int i, k;
   char leftHalf[8], rightHalf[8];
   str.toCharArray(char_array, str_len); 
          for(i = 0; i < 8; i++) {
          leftHalf[i]= char_array[i];
            }
          leftHalf[i] = '\0';

          for(i = 8, k = 0; i < 16; i++, k++) {
          rightHalf[k]= char_array[i];
           }
   encryptplt(leftHalf); // Encryption function
   ...
   delay(24);
   encryptplt(rightHalf); // Encryption function
